...
def main():
    ...
    # Filter to only undersampled classes (< min_samples)
    class_counts = {k: v for k, v in class_counts.items() if v < args.min_samples}
    logging.info(f"Filtered to {len(class_counts)} undersampled classes (< {args.min_samples} samples)")
